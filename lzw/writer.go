...
/*
Changes made from original code, where:
- lzw for tiff slightly different than standard lzw
- also we need to write the initial clearCode 256 according
to TIFF spec
*/
...
func (e *encoder) incHi() error {
	...
	// lzw for tiff slightly different than standard lzw
	// with one extra bit off in hi when check against
	// overflow & maxCode
	if e.hi+1 == e.overflow {
		e.width++
		e.overflow <<= 1
	}
	if e.hi+1 == maxCode {
...
func NewWriter(w io.Writer, order Order, litWidth int) io.WriteCloser {
	...
	// Without this, gdal_translate complains:
	// LZWDecode:Corrupted LZW table at scanline 0
	// This isn't included in the standard lib.
	// Could be specific to LZW of GeoTiff, according to the spec:
